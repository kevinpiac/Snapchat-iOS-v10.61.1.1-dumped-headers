//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "SCComposerViewOwner-Protocol.h"
#import "SCMapCurrentStatusContext-Protocol.h"
#import "SCMapModalViewControllerSubclass-Protocol.h"
#import "SCMapPeopleFriendsListener-Protocol.h"
#import "SCMapStatusFetchingListener-Protocol.h"

@class NSString, SCMapCurrentStatusDetailView, SCMapExploreDataStoreV1, SCMapExploreMyStatus, SCMapLoggerEventSender, SCMapModalViewController;
@protocol SCMapCurrentStatusViewControllerDelegate, SCMapPeopleFriendsProvider, SCMapStatusCreationViewProvider;

@interface SCMapCurrentStatusViewController : UIViewController <SCComposerViewOwner, SCMapCurrentStatusContext, SCMapPeopleFriendsListener, SCMapStatusFetchingListener, SCMapModalViewControllerSubclass>
{
    id <SCMapCurrentStatusViewControllerDelegate> _delegate;
    SCMapLoggerEventSender *_eventSender;
    SCMapExploreDataStoreV1 *_exploreDataStore;
    id <SCMapPeopleFriendsProvider> _mapPeopleFriendsProvider;
    SCMapExploreMyStatus *_myStatus;
    unsigned long long _statusSessionId;
    id <SCMapStatusCreationViewProvider> _viewProvider;
    SCMapCurrentStatusDetailView *_currentStatusDetailView;
    _Bool _isDeletingStatus;
    _Bool _isMostRecentStatus;
}

- (void).cxx_destruct;
- (void)_reloadViewers;
- (void)_updateIsMostRecentStatus;
- (id)_viewerViewModelsForViewerUserIds:(id)arg1;
- (id)composerWillCreateViewForClass:(Class)arg1 nodeId:(id)arg2;
- (id)defaultProjectNameV2;
- (void)didLongPressViewerWithUserId:(id)arg1;
- (void)didTapAvatarOfViewerWithUserId:(id)arg1 avatarView:(id)arg2;
- (void)didTapDeleteButton;
- (void)didTapDismissButton;
- (id)initWithDelegate:(id)arg1 eventSender:(id)arg2 exploreDataStore:(id)arg3 mapPeopleFriendsProvider:(id)arg4 myStatus:(id)arg5 statusSessionId:(unsigned long long)arg6 viewProvider:(id)arg7;
- (void)mapStatusFetcherDidLoadMyStatus:(id)arg1;
- (_Bool)modalViewController:(id)arg1 panGestureRecognizer:(id)arg2 shouldBeginInView:(id)arg3;
- (void)peopleFriendsProvider:(id)arg1 didUpdateStoryForUserIds:(id)arg2;
- (void)peopleFriendsProviderDidUpdate:(id)arg1;
- (id)toJavaScript;
- (void)viewDidLoad;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(nonatomic) __weak SCMapModalViewController *parentModalViewController;
@property(readonly) Class superclass;

@end

