...
@class NSArray, NSMutableDictionary, NSSet, NSString, SCAPIClient, SCDisplayContext, SCDownloadConsumptionRecorder, SCLazy, SCNetworkConnectivityChangeNotifier, SCNetworkRequestRankingHandler, SCNetworkRequestRankingLogger, SCNetworkUiPageChangeNotifier, SCRequestConcurrencyController, SCRequestManagerLogger, SCRequestManagerRunningTaskState, SCRequestPriorityConcurrencyController, SCRequestTaskPool;
@protocol SCNetworkInterceptor, SCPerforming, SCRequestSchedulerDelegate, SCRequestTaskRunDelegate;

@interface SCRequestScheduler : NSObject <SCNNetworkNetworkManager, SCNNetworkCallbackDelegate>
{
    ...
    id <SCPerforming> _requestSigningPerformer;
...
- (void).cxx_destruct;
- (void)_addContext:(id)arg1 toRequestTask:(id)arg2 andRun:(_Bool)arg3;
...
- (void)_handleUserInitiatedRequestTask:(id)arg1 andRun:(_Bool)arg2;
...
- (void)didRunTask:(id)arg1 withData:(id)arg2;
...
@property(retain, nonatomic) id <SCPerforming> requestSigningPerformer; // @synthesize requestSigningPerformer=_requestSigningPerformer;
- (void)reset;
