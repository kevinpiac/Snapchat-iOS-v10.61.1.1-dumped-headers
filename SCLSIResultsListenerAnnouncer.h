//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SCLSIResultsListener-Protocol.h"

@class NSString;

@interface SCLSIResultsListenerAnnouncer : NSObject <SCLSIResultsListener>
{
    struct mutex _mutex;
    struct shared_ptr<std::__1::vector<__weak id<SCLSIResultsListener>, std::__1::allocator<__weak id<SCLSIResultsListener>>>> _listeners;
}

- (id).cxx_construct;
- (void).cxx_destruct;
- (_Bool)addListener:(id)arg1;
- (void)backend:(id)arg1 request:(id)arg2 receivedError:(id)arg3;
- (void)backend:(id)arg1 request:(id)arg2 receivedResponse:(id)arg3;
- (void)backend:(id)arg1 willProcessRequest:(id)arg2;
@property(readonly, copy) NSString *description;
- (void)removeListener:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

