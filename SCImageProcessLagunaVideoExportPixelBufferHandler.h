//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SCImageProcessVideoExportPixelBufferHandler.h"

@class AVAssetWriter, NSArray, SCImageProcessAssetWriterInputPixelBufferAdaptor, SCImageProcessQueue;

@interface SCImageProcessLagunaVideoExportPixelBufferHandler : SCImageProcessVideoExportPixelBufferHandler
{
    SCImageProcessQueue *_imageProcessQueue;
    AVAssetWriter *_assetWriter;
    SCImageProcessAssetWriterInputPixelBufferAdaptor *_pixelBufferAdaptor;
    NSArray *_circleGPUCommands;
    NSArray *_circleCPUCommands;
}

- (void).cxx_destruct;
- (void)_appendOutputPixelBuffer:(struct __CVBuffer *)arg1 presentationTime:(CDStruct_1b6d18a9)arg2 semaphore:(id)arg3;
- (id)initWithImageProcessQueue:(id)arg1 GPUCommands:(id)arg2 CPUCommands:(id)arg3 leftStereoGPUCommands:(id)arg4 rightStereoGPUCommands:(id)arg5 assetReader:(id)arg6 assetWriter:(id)arg7 readerOutput:(id)arg8 writerInput:(id)arg9 pixelBufferAdaptor:(id)arg10 stereoPixelBufferAdaptor:(id)arg11 orientation:(long long)arg12 viewportTransform:(struct CGAffineTransform)arg13 cpuBufferTransform:(struct CGAffineTransform)arg14 queue:(id)arg15 firstSampleBuffer:(struct opaqueCMSampleBuffer *)arg16 maxFrameRate:(int)arg17 presentationTimeScale:(double)arg18 videoSourceSize:(struct CGSize)arg19 videoCircleColor:(id)arg20 videoCirclePadding:(struct CGSize)arg21 videoCircleMask:(_Bool)arg22 magicMomentActive:(_Bool)arg23 magicMomentFrames:(long long)arg24;

@end

